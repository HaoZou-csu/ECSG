...
def results_regression(
    ...
    if ensemble_folds == 1:
        df.to_csv(
            index=False,
            path_or_buf=(f"results/{name}_results_{model_name}_r-{run_id}.csv"),
        )
    else:
        df.to_csv(
...
def results_classification(
    ...
    # NOTE we save_old pre_logits rather than logits due to fact that with the
    ...
    results_logits = {}
    for n_ens, y_logit in enumerate(y_logits):
        pred_dict = {
            f"class-{lab}-pred_{n_ens}_pro": val for lab, val in enumerate(y_logit.T)
        }
        results_logits.update(pred_dict)
        if model.robust:
            ale_dict = {
                f"class-{lab}-ale_{n_ens}": val
                for lab, val in enumerate(y_pre_ale[n_ens].T)
            }
            results_logits.update(ale_dict)

    df = pd.DataFrame({**core, **results, **results_logits})

    if ensemble_folds == 1:
        df.to_csv(
            index=False,
            path_or_buf=(f"results/Roost/test_results_{model_name}_r-{run_id}.csv"),
        )
    else:
        df.to_csv(
            index=False, path_or_buf=(f"results/Roost/ensemble_results_{model_name}.csv")
        )

def train_results_classification(
        model_class,
        model_name,
        run_id,
        ensemble_folds,
    ...
    if ensemble_folds == 1:
        df.to_csv(
            index=False,
            path_or_buf=(f"results/Roost/train_results_{model_name}_r-{run_id}.csv"),
        )
    else:
        df.to_csv(
            index=False, path_or_buf=(f"results/Roost/ensemble_results_{model_name}.csv")
...
def val_results_classification(
        model_class,
        model_name,
        run_id,
        ensemble_folds,
    ...
    if ensemble_folds == 1:
        if predict:
            df.to_csv(
                index=False,
                path_or_buf=(f"results/Roost/predict_results_{model_name}_r-{run_id}.csv"),
            )
        else:
            df.to_csv(
                index=False,
                path_or_buf=(f"results/Roost/val_results_{model_name}_r-{run_id}.csv"),
            )
    else:
        df.to_csv(
            index=False, path_or_buf=(f"results/Roost/ensemble_results_{model_name}.csv")
...
def predict_results_classification(
        model_class,
        model_name,
        run_id,
        ensemble_folds,
