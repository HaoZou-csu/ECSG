...
def train_results_classification(
    ...
    if ensemble_folds == 1:
        df.to_csv(
            index=False,
            path_or_buf=(f"results/train_results_{model_name}_r-{run_id}.csv"),
        )
    else:
        df.to_csv(
            index=False, path_or_buf=(f"results/ensemble_results_{model_name}.csv")
...
def val_results_classification(
    ...
    if ensemble_folds == 1:
        if predict:
            df.to_csv(
                index=False,
                path_or_buf=(f"results/predict_results_{model_name}_r-{run_id}.csv"),
            )
        else:
            df.to_csv(
                index=False,
                path_or_buf=(f"results/val_results_{model_name}_r-{run_id}.csv"),
            )
    else:
        df.to_csv(
            index=False, path_or_buf=(f"results/ensemble_results_{model_name}.csv")
